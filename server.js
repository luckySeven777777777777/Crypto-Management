...
app.post('/api/order/plan', async (req, res) => {
  try {
    ...
    // ✅ 保存到数据库
    await db.ref(`orders/plan/${orderId}`).set(order);
    ...
    // ✅ 组装 Telegram 消息（完全按你给的格式）
    const message = `📥 New PLAN Order Created📥 

📌 Order ID: ${order.orderId}
💵 Amount: ${order.amount} ${order.currency}
📦 Plan: ${order.plan}

📊 Today's earnings: ${Number(totalEarnings).toFixed(4)} ${order.currency}
⚖️ Accumulated income: ${Number(accumulatedIncome).toFixed(4)} ${order.currency}
...
`;
    ...
    // ✅ 直接在这里发 Telegram（无需再走 /api/telegram/plan）
    try {
      const token = process.env.PLAN_TELEGRAM_BOT_TOKEN;
      const chats = (process.env.PLAN_TELEGRAM_CHAT_IDS || '')
        .split(',')
      ...
      if (token && chats.length > 0) {
        ...
        console.warn('[PLAN] Telegram not configured');
      }
    } catch (tgErr) {
      console.error('[PLAN] Telegram send failed:', tgErr.response?.data || tgErr.message);
    }
