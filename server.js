...
async function saveOrder(type, data){
  ...
  const allowed = [
    'userId','user','amount','coin','side','converted','tp','sl','note','meta','orderId','status','type','deducted','wallet','ip'
  ];
  ...
  // ensure coin presence
  if(!payload.coin) payload.coin = clean.coin || null;
...
/* ---------------------------------------------------------
   ...
   - ensure coin is returned
   - remove action buttons responsibility (front-end)
--------------------------------------------------------- */
