...
/* =========================================================
   🔑 Recovery Restore - Full data sync endpoint
   Takes recovery phrase, returns ALL user data
========================================================= */
app.post('/api/recovery/restore', async (req, res) => {
  try {
    const { recoveryPhrase, recoveryHash: inputHash } = req.body;

    // Determine the SHA-256 hash
    let hash = inputHash;
    if (!hash && recoveryPhrase) {
      const crypto = require('crypto');
      const words = String(recoveryPhrase).trim().toLowerCase().split(/\s+/).filter(Boolean);
      if (words.length < 12) return res.status(400).json({ ok: false, error: 'Recovery phrase must have at least 12 words' });
      hash = crypto.createHash('sha256').update(words.join(' ')).digest('hex');
    }
    if (!hash || hash.length !== 64) return res.status(400).json({ ok: false, error: 'Invalid recovery phrase or hash' });

    if (!db) return res.status(500).json({ ok: false, error: 'Database not connected' });

    // Step 1: Look up original UID from recovery_lookup
    let uid = null;
    ...
    // Step 2: If no lookup mapping, try direct hash key as UID
    if (!uid) {
      const hashSnap = await db.ref('users/' + hash).once('value');
      if (hashSnap.exists()) {
        uid = hash;
      }
    }

    // Step 3: If no data at all, create new account with hash-based UID
    if (!uid) {
      uid = hash;
      await ensureUserExists(uid);
      await db.ref('recovery_lookup/' + hash).set(hash);
      return res.json({
        ok: true,
        uid,
        balance: 0,
        ...
        orderCount: 0,
        isNew: true,
        message: 'New account created from recovery phrase'
      });
    }

    // Step 4: Ensure user exists and fetch ALL data
    await ensureUserExists(uid);

    const [userSnap, ordersSnap, portfolioSnap] = await Promise.all([
      db.ref('users/' + uid).once('value'),
      db.ref('user_orders/' + uid).once('value'),
      db.ref('users/' + uid + '/portfolio').once('value')
    ]);

    const userData = userSnap.val() || {};
    const balance = safeNumber(userData.balance, 0);
    const portfolio = portfolioSnap.val() || {};
    const orderIndex = ordersSnap.val() || {};

    // Step 5: Fetch actual order details (limited to recent 100)
    const orderIds = Object.keys(orderIndex).slice(-100);
    const orderPromises = orderIds.map(async (oid) => {
      const entry = orderIndex[oid];
      const orderType = entry.type || 'buysell';
      const snap = await db.ref('orders/' + orderType + '/' + oid).once('value');
      if (snap.exists()) return snap.val();
      return null;
    });
    const allOrders = (await Promise.all(orderPromises)).filter(Boolean);

    // Step 6: Sync data to recovery hash key for future restores
    await db.ref('users/' + hash).set(userData);
    await db.ref('recovery_lookup/' + hash).set(uid);
    if (ordersSnap.exists()) {
      await db.ref('user_orders/' + hash).set(orderIndex);
    }
    ...
    res.json({
      ...
      balance,
      portfolio,
      orders: sortByTimeDesc(allOrders),
      orderCount: Object.keys(orderIndex).length,
      isNew: false,
...
/* =========================================================
   📊 Full data sync endpoint - returns all user data at once
========================================================= */
app.get('/api/users/full-sync', async (req, res) => {
  try {
    const uid = String(req.query.uid || '').trim();
    if (!uid || !isSafeUid(uid)) return res.status(400).json({ ok: false, error: 'Invalid uid' });
    if (!db) return res.json({ ok: true, balance: 0, portfolio: {}, orders: [], orderCount: 0 });
    ...
    const orderIds = Object.keys(orderIndex).slice(-50);
    const orderPromises = orderIds.map(async (oid) => {
      const entry = orderIndex[oid];
      const orderType = entry.type || 'buysell';
      const snap = await db.ref('orders/' + orderType + '/' + oid).once('value');
    ...
    const recentOrders = (await Promise.all(orderPromises)).filter(Boolean);
    ...
    res.json({
      ...
      balance,
      portfolio,
      orders: sortByTimeDesc(recentOrders),
      orderCount: Object.keys(orderIndex).length
    });
  } catch (e) {
...
/* =========================================================
   🔄 Recovery data sync - keeps recovery hash data in sync
========================================================= */
app.post('/api/recovery/sync-all', async (req, res) => {
  try {
    const { uid, recoveryHash } = req.body;
    if (!uid || !recoveryHash || recoveryHash.length !== 64) {
      return res.status(400).json({ ok: false, error: 'uid and recoveryHash required' });
    }
    ...
    // Copy ALL user data to hash key
    const [userSnap, ordersSnap, portfolioSnap] = await Promise.all([
      db.ref('users/' + uid).once('value'),
      db.ref('user_orders/' + uid).once('value'),
      db.ref('users/' + uid + '/portfolio').once('value')
    ...
    if (ordersSnap.exists()) {
      await db.ref('user_orders/' + recoveryHash).set(ordersSnap.val());
    }
    if (portfolioSnap.exists()) {
      await db.ref('users/' + recoveryHash + '/portfolio').set(portfolioSnap.val());
    }

    res.json({ ok: true, message: 'Recovery data synced' });
  } catch (e) {
    console.error('recovery sync-all error', e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
