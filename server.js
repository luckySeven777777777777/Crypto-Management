...
async function saveOrder(type, data){
  ...
  const allowed = [
    ...
    'tp','sl','note','meta','orderId','status','type','deducted','wallet','ip','currency'
  ];
  ...
  try{
    // generic event for admin UI
    broadcastSSE({ type:'new', typeName:type, order: payload });
  }catch(e){}
  try{
    // specific event for buysell wallet listeners
    if (type === 'buysell') {
